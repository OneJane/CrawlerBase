    ...
    if len(origin_res_list) > 0:
        ...
        for origin_res in origin_res_list:
            try:
                ...
                if float(wencai['竞价涨幅']) * float(wencai['竞价换手率']) > 1 and float(wencai['竞价量比']) > 1 and float(wencai['竞价量比']) < 30 \
                        and wencai['竞价额'] > 10000000 and res_json_new["上市天数"] > 50\
                        and (wencai["竞价量"] * wencai["竞价换手率"] / wencai["昨日成交量"]) > 0.01 and wencai['竞价涨幅']<9:
        ...
        #
        # # 竞昨比高 换手率应该低
        # res_list.sort(key=lambda x: (x['竞价换手率']*x["竞价量比"]/x['竞价涨幅']),reverse=True) # 169 78
        # if len(res_list) > 0:
        ...
        res_list.sort(key=lambda x: (x['竞价额']*x["竞价量比"]*x["竞价换手率"]*x["昨日成交量"]/x["竞价量"]), reverse=True)  #
        if len(res_list) > 0:
...
"2022年11月17日_上市天数，2022年11月17日竞价实际换手率大于1%，2022年11月17日竞价涨幅，2022年11月16日成交量，2022年11月17日竞价换手率，2022年11月17日竞价量比，主板，2022年11月17日竞价额，2022年11月17日涨跌幅排序，非st，2022年11月17日竞价量",
    ...
    file_object1 = open("liti5.txt", 'r', encoding='UTF-8')
    try:
        while True:
